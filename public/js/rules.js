...
(function (root, factory) {
  ...
  const C = {
    ...
    * 取 1 單位 ＝ 13.7 公分，理由是這樣其他尺寸都說得通：
    *   路寬平均 130 單位 ＝ 17.8 公尺（八隻並排的卡通賽道，合理）
    *   碰撞半徑 11 單位 ＝ 1.5 公尺（毛毛蟲本人就這麼大隻）
    * 換出來的速度也落在賽車該有的範圍：
    *   基礎速度 152 ＝ 75 km/h｜泥巴 0.45 倍 ＝ 34｜果汁＋加速帶最快約 146
    * 這個值只影響顯示，不影響任何物理。 */
    UNIT_M: 0.137061404,
    ...
    /* 速度 */
    BASE_SPEED: 152,           /* 跑道上的基礎速度（單位／秒，儀表約 75 km/h）。按住前進就是這個速度。
    ...
    /* 轉向 */
    /* 低速時的最大角速度（弧度／秒）。車速提高到 75 km/h 後，
     * 仍要保留足夠的轉向能力才能通過既有彎道；較慢的角加速度則負責
     * 把快速點按造成的畫面搖晃壓下來。 */
    TURN: 2.35,
     ...
     *
     * 4.5（0.52 秒滿舵）保留重量感，快速點放也不會把整個畫面甩來甩去。
     * 持續過彎仍然轉得到滿舵，只是保留一點反應時間，
     * 跑起來變成「有重量」而不是「一點就飄」。
     *
     * 不能再往下，否則會有賽道過不了彎，AI 也開始撞牆。
