...
(function (root, factory) {
  ...
  const C = {
    ...
    * 這個值只影響顯示，不影響任何物理。 */
    ...
    /* 速度 */
    BASE_SPEED: 152,           /* 跑道上的基礎速度（單位／秒）。按住前進就是這個速度。
    ...
    /* 低速時的最大角速度（弧度／秒）。
     *
     * 這個值就是「畫面最快轉多快」—— head 模式的鏡頭鎖在車頭上，
     * 車頭轉多快畫面就轉多快。3.15 高速時是 135°/s，滿舵會暈。
     *
     * 調鏡頭的轉速上限沒有用（實測上限從 54 降到 14°/s，持續過彎的尖峰
     * 只從 119 掉到 113°/s，落後卻從 25° 爆到 94°）—— 彎要轉過去的總角度是
     * 賽道決定的，鏡頭遲早要轉完。真正有效而且「零代價」的是讓車本身轉慢一點：
     * 畫面跟著少轉，而鏡頭完全不用落後。
     *
     * 2.35 高速時是 101°/s（少 25%），二十五張賽道全部還跑得完，圈速幾乎不變。
     * 再往下到 2.10 就有一張賽道過不了彎。 */
    TURN: 2.35,
    ...
    /* 角加速度（弧度／秒²）。約 TURN/TURN_ACCEL 秒到滿舵。
     *
     * 這個值決定「左右修一下方向，畫面會擺多大」—— 鏡頭鎖在車頭上，
     * 車頭甩多少畫面就擺多少。8.5（0.28 秒滿舵）太靈敏：
     * 以 0.2 秒為週期左右交替打方向，車頭峰對峰擺 56.8 度，整個畫面就跟著搖。
     *
     * 4.5（0.52 秒滿舵）把同樣的擺幅砍到 30.1 度，快速點放（0.2 秒週期）
     * 從 13.0 砍到 6.9 度。持續過彎完全不受影響 —— 還是轉得到滿舵，只是晚 0.24 秒，
     * 跑起來變成「有重量」而不是「一點就飄」。
     *
     * 不能再往下：3.5 會有一張賽道過不了彎，AI 也開始撞牆。
     * 回正速度（TURN_RELEASE）刻意留著比較快，鬆手就自己打直，殘餘的擺動才收得掉。 */
    TURN_ACCEL: 4.5,
