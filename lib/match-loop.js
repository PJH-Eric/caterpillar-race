...
function createLoop(hub, io, opt) {
  ...
  function startRace(room) {
    ...
    for (const m of seats) inputs.set(m.id, { steer: 0, use: false });
  ...
  function setInput(personId, msg) {
    const prev = inputs.get(personId) || { steer: 0, use: false };
    inputs.set(personId, {
