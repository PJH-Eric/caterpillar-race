...
function createLoop(hub, io, opt) {
  ...
  function startRace(room) {
    ...
    for (const m of seats) inputs.set(m.id, { steer: 0, gas: 0, use: false });
  ...
  function tick() {
    ...
    for (const room of Array.from(hub.rooms.values())) {
      ...
      for (const r of room.state.racers) {
        frame[r.id] = inputs.get(r.id) || { steer: 0, gas: 0, use: false };
      }
      Rules.step(room.state, frame);
      /* use 是邊緣觸發，用掉就收回，不然會一路連發 */
      for (const id of inputs.keys()) {
        const it = inputs.get(id);
        if (it && it.use) inputs.set(id, { steer: it.steer, gas: it.gas || 0, use: false });
      }
  ...
  function setInput(personId, msg) {
    const prev = inputs.get(personId) || { steer: 0, gas: 0, use: false };
    inputs.set(personId, {
