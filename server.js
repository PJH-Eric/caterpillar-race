 ...
 *   lib/protocol.js    客戶端訊息 → 上面兩支的呼叫，權限檢查也在那裡
...
const GAME_ID = 'caterpillar-race';
const MILESTONE = 'M2';
/* 多久沒收到任何 frame 才算連線可能死了（要比心跳間隔寬鬆一點） */
const ALIVE_MS = 2500;
...
const server = http.createServer((req, res) => {
  ...
  if (url === '/api/rooms') {
    json(res, 200, { rooms: hub.listRooms(), seats: hub.CONST.SEATS_MAX });
    return;
  }
...
ws.attach(server, {
  ...
  onConnection(socket) {
    ...
    socket.sendJSON({
      type: 'hello', game: GAME_ID, milestone: MILESTONE, personId: person.id,
      seats: hub.CONST.SEATS_MAX, spectators: hub.CONST.SPECTATORS, quick: hub.QUICK
    });
    ...
    socket.on('close', () => {
      ...
      /* 對局中掉線＝幽靈化，30 秒內回來可以接回原位（規劃書 §8.3） */
      const res = hub.markDisconnected(person.id);
      loop.forget(person.id);
      if (res && res.room && !res.closed) loop.sendRoom(res.room);
      loop.sendLobby(true);
    });
  }
});
...
if (require.main === module) {
  server.listen(PORT, () => {
    console.log('毛毛蟲賽車（' + MILESTONE + '：線上對戰）已啟動');
